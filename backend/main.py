...
def main(json_output=False):
    """
    If json_output=True the function will suppress ordinary prints and return
    a list-of-dicts (records) representing the timetable. Otherwise it behaves
    as before (prints logs + interactive prompt).
    """
    # If we are producing machine-readable JSON, suppress print() to avoid log noise.
    _orig_print = None
    if json_output:
    ...
    try:
        ...
        # If in JSON mode, return records directly (no interactive prompt)
        if json_output:
        ...
        # otherwise behave as before (print table + interactive student lookup)
...
# ---------- Shared solved-cache ----------
# Will hold the last solved result when main(json_output=True) is called.
SOLVED_RESULT = None

def set_solved_result(result: dict):
    """Normalize and store the solver result into SOLVED_RESULT."""
    global SOLVED_RESULT
    if result is None:
        SOLVED_RESULT = None
        return

    # If user passed the old list-of-records directly, normalize to dict:
    if isinstance(result, list):
        SOLVED_RESULT = {
            "records": result,
            "core_batches": {},        # best-effort empty fallback
            "elective_groups": {}
        }
    elif isinstance(result, dict):
        # Expect the dict to include 'records' key as produced by main(..., json_output=True)
        SOLVED_RESULT = {
...
# ---------- Student timetable accessor (unchanged except small guard) ----------
